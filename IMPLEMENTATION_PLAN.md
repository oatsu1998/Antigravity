...
2. **Modular Frontend**: Vanilla JavaScript components with central state management via state-manager.js and universal header via 
av.js.
...
- **Wind Flag Indicator**: Renders a 14px cyan flag SVG rotated using 	ransform: rotate(deg) matching the meteorological wind direction with cardinal direction text (NW, SE, etc.).
- **Dome Filtering**: Indoor venues (indoor: true) do not render weather chips.

### NFL Player Props Integration (pi/kalshi-props.js & props.html)
...
- Safely references etSlip.length using a 	ypeof guard (	ypeof betSlip !== 'undefined' && Array.isArray(betSlip)) inside 
enderLiveWagersPanel() to prevent ReferenceError crashes during initial page render.

### Line Tracker History Fix (line-tracker.html)
- Defined missing etchHistory() function in line-tracker.html to handle historical odds matrix requests.
